...
(async function () {
  ...
  // ── Track a view (for the homepage "Most Read" list) ──────
  // Best-effort — if this fails (e.g. the view_count column hasn't
  // been added yet), the rest of the page still works normally.
  _supabase
    .from('articles')
    .update({ view_count: (article.view_count || 0) + 1 })
    .eq('id', article.id)
    .then(function (res) { if (res.error) console.warn('View count not updated:', res.error.message); });
